...
const Recs = () => {
  ...
  const LOGIN_TOOLTIP =
    "Log in to generate recommendations based on liked songs.";
  const shouldShowPrevRecs = () =>
    currentUser &&
    ((!generatedNewRecs && currentUser.recommendations.length > 0) ||
      (generatedNewRecs && currentUser.recommendations.length > 1));

  useEffect(() => {
    // fetch genres + topTen if not already available
    if (genres.length === 0) {
  ...
  const handleGetRecsByGenres = () => {
    if (selectedGenres.length === 0) {
      alert(
        "You must select at least one genre before recommendations can be generated."
      );
    } else {
  ...
  const handleGetRecsByLikedSongs = () => {
    if (currentUser.likes.length === 0) {
      alert(
        "You must like at least one song before recommendations can be generated based on likes."
      );
    } else {
      setGeneratedNewRecs(true);
      ...
      if (currentUser.likes.length >= 5) {
        const shuffledLikes = [...currentUser.likes].sort(
          (a, b) => 0.5 - Math.random()
        );
  ...
  return (
    <>
      ...
      <div className="btn-group" role="group">
        ...
        <label
          className={`btn ${
            recMethod === "genres" ? "btn-secondary" : "btn-outline-secondary"
          }`}
          htmlFor="by-genres"
        >
          By Selected Genres
        ...
        <label
          className={`btn ${
            recMethod === "songs" ? "btn-secondary" : "btn-outline-secondary"
          }`}
          htmlFor="by-liked-songs"
        >
          By Liked Songs
        </label>
      </div>
        ...
        <>
          {!currentUser && (
            <i
              ...
              id="by-likes-disabled"
              data-tooltip-content={LOGIN_TOOLTIP}
            ></i>
          )}
          <br />
          ...
          <p>
            Select multiple options by holding Ctrl/Cmd and clicking. A maximum
            of five genres can be selected.
          </p>
          <span>Selected genres:</span>
          {selectedGenres &&
            selectedGenres.map((genre, idx) => (
              <span className="badge rounded-pill text-bg-info m-1" key={idx}>
                {genre}
              </span>
            ))}
      ...
      <button
        ...
        onClick={
          recMethod === "genres"
            ? handleGetRecsByGenres
            : handleGetRecsByLikedSongs
        }
      >
            ...
            <>
              ...
              <div className="col">
                <ul className="list-group">
                  <RenderSongsList
                    songs={currentUser.recommendations.at(-1).songs}
                  />
                </ul>
              </div>
            </>
